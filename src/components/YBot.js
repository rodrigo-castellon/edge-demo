...
export function createYBotComponent(path, playElement, startTimestamp) {
    function YBot(props) {
        let group = useRef();
        let { nodes, materials, animations } = useGLTF(path);
        ...
        if (playElement) {
            useEffect(() => {
                ...
                let k = keys[0];
                actions.actions[k].time = startTimestamp;
